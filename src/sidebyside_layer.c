...
{
    fprintf(stderr,"SideBySide Layer: Split and side by side channels in %d groups. gap = %d\n", splits, gap);
    layer l = {0};
    l.type = SIDEBYSIDE;
    l.batch = batch;
    l.w = w;
...
void forward_sidebyside_layer_gpu(const layer l, network_state state)
{
    ...
    for (b=0;b<l.batch;b++)
    {
        // clear, in case of gap
        const_ongpu(l.out_h*l.out_w*l.out_c, 0, l.output_gpu+b*l.outputs, 1);
        ...
        for (c=0;c<l.c;c++)
        {
            ...
            copy_ongpu(l.w*l.h, state.input+b*l.inputs+l.w*l.h*c, 1,
                                l.output_gpu+b*l.outputs+newC*l.out_h*l.out_w+sidepos*l.w*l.h+l.gap*l.w*sidepos, 1);
        }
    }
...
void backward_sidebyside_layer_gpu(const layer l, network_state state)
{
    ...
    for (b=0;b<l.batch;b++)
    {
        ...
        for (c=0;c<l.c;c++)
        {
            int sidepos = c / numInSplit;
            int newC = c % numInSplit;
            axpy_ongpu(l.w*l.h, 1, l.delta_gpu+b*l.outputs+newC*l.out_h*l.out_w+sidepos*l.w*l.h+l.gap*l.w*sidepos, 1,
                                   state.delta+b*l.inputs+l.w*l.h*c, 1);
        }
    }
}
